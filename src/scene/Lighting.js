...
export function setupLighting(scene) {
  // Ambient — minimalan
  const ambient = new THREE.AmbientLight(0xc8e8f5, 1.5);
  scene.add(ambient);

  // Key Light — hladno bijela, odozgo-lijevo
  const keyLight = new THREE.SpotLight(0xffffff, 5);
  ...
  const fillLight = new THREE.PointLight(0x5FABDB, 2.0, 16);
  fillLight.position.set(5, 1, 3);
  scene.add(fillLight);

  // Rim Light — naglašava ivice komponenti
  const rimLight = new THREE.SpotLight(0x3d8ec2, 3.5);
  ...
  const accent = new THREE.PointLight(0x8fcde8, 1.0, 12);
